...
Widget build(BuildContext context, WidgetRef ref) {
  ...
  final carStatusAsync = ref.watch(carroStatusProvider(carId)); // carStatus agora é AsyncValue
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  // Use o carStatusAsync quando for exibir o status
                  carStatusAsync.when(
                    data: (status) => Text(
                      'Status: ${status ?? "Indefinido"}',
                      style: TextStyle(
                        fontSize: 16,
                        color: (status == 'Disponível') ? CoresTheme.corVerde : Colors.red,
                        fontWeight: FontWeight.bold,
                      ),
                    ),
                    loading: () => const CircularProgressIndicator(),
                    error: (error, stack) => const Text('Erro ao carregar o status'),
                  ),
                  Text(
